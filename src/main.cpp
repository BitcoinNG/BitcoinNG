...
#if defined(NDEBUG)
# error "BitcoinNG cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x31769f290ba8856b85e790b0228c82278608603295450bc42d139d1d07cf9849");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BitcoinNG: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BitcoinNG Signed Message:\n";
...
{
    // BitcoinNG: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 5040000); // BitcoinNG: 5m 40k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // BitcoinNG: 1 day
static const int64 nTargetSpacing = 1.25 * 60; // BitcoinNG: 75 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BitcoinNG: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BitcoinNG: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb270d3e766d2e1c19d8be0e879160cb2385c8217c76a3f9aa5624dfa645d9abe");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Ostfriesen-Zeitung 05/Feb/2018 Milchbauern wollen Ersatz fuer die Weidepraemie";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 200 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04712f94a22f832ef2dd98d836fb0ada81df84ac1b6f57dbfa401db2bd322027b28005b537b5c47922804dd226ac9708bfcb73c9778e1b752805f4640b395f9007") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517939803;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087060757;

        if (fTestNet)
        {
            block.nTime    = 1517939839;
            block.nNonce   = 387287340;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x05f93d47c8bd15cd0ed872929387e1af96159f7aecd62b3988608ebb4d5ec6df"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // BitcoinNG: increase each by adding 3 to litecoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitcoinNGMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitcoinNGMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitcoinNGMiner(CWallet *pwallet)
{
    printf("BitcoinNGMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoinng-miner");
    ...
    try { loop {
        ...
        printf("Running BitcoinNGMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitcoinNGMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoinNGMiner, pwallet));
}
